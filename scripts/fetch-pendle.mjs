...
const markets = await Promise.all(addresses.map(async (address) => {
  ...
  for (const value of [data.yt?.price?.usd, data.accountingAsset?.price?.usd, data.impliedApy, data.aggregatedApy, data.underlyingApy]) {
    if (!Number.isFinite(value) || value < 0) throw new Error('Invalid Pendle market');
  }
  return { address, source, expiry: data.expiry, updatedAt: data.dataUpdatedAt, priceUpdatedAt: data.yt.priceUpdatedAt, ytPrice: data.yt.price.usd, accountingPrice: data.accountingAsset.price.usd, ptApy: data.impliedApy, lpApy: data.aggregatedApy, underlyingApy: data.underlyingApy, ytYieldApy: data.underlyingApy };
}));
