...
const server=createServer((req,res)=>{const path='public'+new URL(req.url,'http://localhost').pathname.replace(/\/$/,'/index.html');try{res.setHeader('Content-Type',path.endsWith('.js')?'text/javascript':path.endsWith('.css')?'text/css':path.endsWith('.json')?'application/json':'text/html');res.end(fs.readFileSync(path));}catch{res.statusCode=404;res.end();}}).listen(4186);
...
await page.fill('#strategyAmountInput','0');assert.equal(await page.textContent('#strategyProfit'),'$0.00');
await page.fill('#strategyAmountInput','10000');await page.fill('#tgeInput','2020-01-01');assert.equal(await page.textContent('#strategyPoints'),'0 PTS');
